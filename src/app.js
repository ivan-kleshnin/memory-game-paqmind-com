...
let {modules, a, button, div, makeDOMDriver, h1, h2, p, span} = require("cycle-snabbdom")
...
let renderCell = curry((i, j, cell) => {
  let cellAttrs = {attrs: {"data-row": i, "data-col": j}}
  console.log(cell[1])
  if (cell[1] == 0) {
    // closed
    return span(".cell", cellAttrs, [
      span(".card.flipper.shaker", {attrs: {"data-state": 0}}, [
        span(".front", ["?"]),
        span(".back", [cell[0]]),
      ])
    ])
    ...
    // opened
    return span(".cell", cellAttrs, [
      span(".card.flipper.shaker.flipped", {attrs: {"data-state": 1}}, [
        span(".front", ["?"]),
        span(".back", [cell[0]]),
      ])
    ])
    ...
    // done
    return span(".cell", cellAttrs, [
      span(".card.flipper.shaker.flipped.hidden", {attrs: {"data-state": 2}}, [
        span(".front", ["?"]),
        span(".back", [cell[0]]),
      ])
    ])
